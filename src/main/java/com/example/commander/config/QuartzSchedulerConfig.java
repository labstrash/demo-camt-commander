...
import com.example.commander.scheduling.AutowiringSpringBeanJobFactory;
...
/**
 * Configures Quartz scheduler with report generation jobs/triggers, the dead-letter recovery
 * job/trigger, and the audit retention job/trigger.
 *
 * <p>Builds job and trigger definitions from {@link SchedulingProperties} (report jobs), {@link
 * MqResilienceProperties} (recovery job cadence), and {@link AuditRetentionProperties}
 * (retention job cadence), and registers all of them with Spring Boot's auto-configured {@code
 * SchedulerFactoryBean} — through a {@link SchedulerFactoryBeanCustomizer} bean: {@code
 * setJobDetails}/{@code setTriggers} are plain replace-the-array setters, so job/trigger
 * registration happens through a single such customizer ({@link #reportJobScheduleCustomizer}),
 * not split across several that would silently discard each other's work.
 *
 * <p>Also wires {@link AutowiringSpringBeanJobFactory} onto the scheduler via a second, simple
 * customizer ({@link #jobFactoryCustomizer}) — {@code spring-boot-starter-quartz}'s
 * autoconfiguration has no {@code JobFactory} auto-detection of its own (unlike, say, its
 * {@code Trigger}/{@code JobDetail} bean pickup), so without this every job class here (none of
 * which have a no-arg constructor) would fail to instantiate the first time Quartz actually
 * tried to fire one.
 *
 * <p>The recovery and retention jobs/triggers live in their own Quartz group ({@link
 * #RECOVERY_GROUP}), separate from {@link ReportJobScheduleBuilder}'s {@code camt-scheduling}
...
public class QuartzSchedulerConfig {
    ...
    /**
     * Wires {@link AutowiringSpringBeanJobFactory} onto the scheduler — without this, Quartz
     * falls back to a plain {@code SpringBeanJobFactory} with no {@code ApplicationContext}
     * awareness, which reflectively instantiates job classes via a no-arg constructor. None of
     * this application's job classes have one (they all take constructor-injected
     * dependencies), so every job would fail to instantiate the first time its trigger fired.
     *
     * @param jobFactory the Spring-aware job factory
     * @return a customizer that sets it on the scheduler
     */
    @Bean
