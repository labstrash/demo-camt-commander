...
public class ReportPipelineItemReader extends AbstractItemStreamItemReader<ReportMessageEnvelope> {
    ...
    private final ReportConfigTreeRepository repository;
    ...
    private long configsReadCount;
    private boolean noMorePages;

    public ReportPipelineItemReader(
            ReportConfigTreeRepository repository,
    ...
    public void open(ExecutionContext executionContext) throws ItemStreamException {
        ...
        configsReadCount = executionContext.getLong(CONFIGS_READ_COUNT_KEY, 0L);
    }

    @Override
    public void update(ExecutionContext executionContext) throws ItemStreamException {
        executionContext.putLong(LAST_SEEN_ID_KEY, lastSeenId);
        executionContext.putLong(CONFIGS_READ_COUNT_KEY, configsReadCount);
    }

    @Override
    public ReportMessageEnvelope read() {
        ...
        ReportMessageEnvelope item = front.items.pollFirst();
    ...
    private void fetchNextPage() {
        ...
        configsReadCount += page.size();

        List<ReportConfigTree> trees = repository.assembleTrees(page);
        for (ReportConfigTree tree : trees) {
            List<ReportMessageEnvelope> messages = fanOutAssemblyService.assemble(tree, contextFor(tree));
